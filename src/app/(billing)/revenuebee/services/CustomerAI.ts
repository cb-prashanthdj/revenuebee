...
export class CustomerAnalyzer {
  ...
  analyzeCustomerData(customers: Customer[]): AnalysisResult {
    ...
    const expiredPaymentMethods = customers.filter(
      (c) => c.paymentMethod?.isExpired
    );
    const soonToExpirePaymentMethods = customers.filter(
      (c) =>
        c.paymentMethod &&
    ...
    const averageMRR =
      activeCustomerCount > 0 ? totalMRR / activeCustomerCount : 0;
  ...
  generateResponse(query: string): AIResponse {
    ...
    // Revenue or MRR query
    else if (q.includes("mrr") || q.includes("money")) {
      const activeCustomers = this.customerData.filter(
        (c) => c.status === "active"
      );
      const topMRRCustomers = [...this.customerData].sort(
        (a, b) => b.mrr - a.mrr
      );
      ...
      response.sections = [
        ...
        },
        {
          ...
          } active customers with an average MRR of $${Math.round(
            analysis.revenueMetrics.averageMRR
          ).toLocaleString()}`,
      ...
      response.recommendation =
        "Consider implementing a tiered customer success program focusing on your top 20% of customers by MRR.";
    }
      ...
      response.sections = [
        ...
        },
        {
          ...
          description: `${
            ...
            analysis.paymentIssues.expiring.count
          } customers have payment method issues`,
          count:
            analysis.paymentIssues.missing.count +
            analysis.paymentIssues.expired.count +
            analysis.paymentIssues.expiring.count,
