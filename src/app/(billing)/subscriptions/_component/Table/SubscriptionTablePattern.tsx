...
interface SubscriptionTablePatternProps {
...
const SubscriptionTablePatternPattern: React.FC<
  SubscriptionTablePatternProps
> = ({ selectors = {}, tabledata, filteredTableData, reset = false }) => {
  ...
  const headerClassNames = {
    subscriptionInfo: "w-0 font-semibold",
    customerInfo: "w-auto",
    customerId: "w-auto",
    nextRenewal: "w-auto",
    MRR: "w-auto",
  ...
  const renderCellContent = (key: string, row: any) => {
    ...
    if (key === "customerInfo") {
      return <CustomerInfoCell customer={row.customer} />;
    } else if (key === "subscriptionInfo") {
      return <SubscriptionInfoCell subscription={row} />;
      ...
      return <div className="text-neutral-600">{value || "-"}</div>;
    }
  };
...
export default SubscriptionTablePatternPattern;
